...
<script>
import { addSkus,getFeedbacksType } from '@/api/index'
...
export default {
  ...
  data(){
	...
	created() {
		this.temp.product_id = this.$route.params && this.$route.params.sid
		console.log(this.temp.product_id)
	},
	methods:{
		updateData(){
			      ...
			      addSkus(this.temp).then( res => {
			        if (res.code == 200) {
			          this.$message({
						...
						this.temp.times = ''
						this.temp.remark = ''
						this.temp.coupon_for_copemate_recommend = ''
						this.temp.coupon_for_user_recommend = ''
						this.temp.fee_reduce_count = ''
			        }else{
						this.$message({
						  message: res.error,
						  type: 'warning'

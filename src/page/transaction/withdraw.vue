<template>
  ...
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column label="提币币种" min-width="90px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.title }}</span>
        </template>
      </el-table-column>
      <el-table-column label="提币地址" min-width="150px" align="center">
        <template slot-scope="scope">
          <el-tag>{{ scope.row.address }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="提币金额" min-width="150px" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column label="提币时间" min-width="150px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.create_time | parseTime() }}</span>
        </template>
      </el-table-column>
      <el-table-column label="提币状态" width="110px" align="center">
        <template slot-scope="scope">
          <el-tag type="success" >{{scope.row.create_time | statusOpFilter}}</el-tag>
        </template>
      </el-table-column>
    ...
    <pagination v-show="total>0" :total="total" :page.sync="listQuery.page" :limit.sync="listQuery.limit" @pagination="getWithDrawList" />
    <el-dialog title="充币详情" :visible.sync="dialogFormVisible" v-bind:style="{height:fullHeight}" customClass="customWH">
      ...
      <el-form ref="dataForm" :model="temp" label-position="left" label-width="110px" style="height=100%;width: 600px; margin-left:50px;">
        ...
        <el-form-item label="提币状态">
          <span>{{ temp.create_time | statusOpFilter }}</span>
        </el-form-item>
      </el-form>
      </el-scrollbar>
...
<script>
import { getWithDrawList } from '@/api/index'
